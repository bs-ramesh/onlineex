        ...
        <div class="app-main__inner">
            ...
            <?php 
                ...
                if($exam_id != "")
                {
                   ...
                   $exam_course = $selEx['branch_id'];

                   

                   $selExmne = $conn->query("SELECT * FROM examinee_tbl et  WHERE branch_id='$exam_course'  ");
                    ...
                    <div class="page-title-wrapper">
                        <div class="page-title-heading">
                            <div><b class="text-primary">RANKING BY EXAM</b><br>
                                Exam Name : <?php echo $selEx['ex_subject']; ?><br><br>
                          ...
                          <tbody>
                            ...
                            <?php 
                                while ($selExmneRow = $selExmne->fetch(PDO::FETCH_ASSOC)) { ?>
                                    <?php 
                                            $exmneId = $selExmneRow['usn'];
                                            $selScore = $conn->query("SELECT * FROM exam_question_tbl eqt INNER JOIN exam_answers ea ON eqt.eqt_id = ea.quest_id AND eqt.exam_answer = ea.exans_answer  WHERE ea.axmne_id='$exmneId' AND ea.exam_id='$exam_id'  ORDER BY ea.exans_id DESC");
                ...
                <div class="main-card mb-3 card">
                    ...
                    <div class="table-responsive">
                        <table class="align-middle mb-0 table table-borderless table-striped table-hover" id="tableList">
                            ...
                            <tbody>
                              <?php 
                                $selExam = $conn->query("SELECT * FROM exam_tbl ORDER BY branch_id DESC ");
                                if($selExam->rowCount() > 0)
                                {
                                    while ($selExamRow = $selExam->fetch(PDO::FETCH_ASSOC)) { ?>
                                        <tr>
                                            <td class="pl-4"><?php echo $selExamRow['ex_subject']; ?></td>
                                            <td>
                                                <?php 
                                                    $courseId =  $selExamRow['branch_id']; 
                                                    $selCourse = $conn->query("SELECT * FROM branch_tbl WHERE branch_id='$courseId' ");
                                                    while ($selCourseRow = $selCourse->fetch(PDO::FETCH_ASSOC)) {
                                                        echo $selCourseRow['branch_name'];
                                                    }
                                                ?>
                                            </td>
